...
-- ================below includestotal sales

SELECT substr(InvoiceDate,1,4) as "Year", COUNT(InvoiceId) as "Number of Invoices", COUNT(*), ROUND(SUM(Total),2) FROM Invoice WHERE InvoiceDate LIKE '%2009%' UNION
SELECT substr(InvoiceDate,1,4), COUNT(InvoiceId), COUNT(*), ROUND(SUM(Total),2) FROM Invoice WHERE InvoiceDate LIKE '%2011%'